controldata_fish <- function(dat, year, amp_abbrv, state_abbrv) {
  ...
  marine_parks <- st_read("data/south-west network/spatial/shapefiles/western-australia_marine-parks-all.shp") %>%
    ...
    # minimal "status" flag for plotting/summaries
  ...
  # ensure we have a sensible CRS when making vectors
  tempdat_v <- terra::vect(as.data.frame(dat, xy = TRUE), geom = c("x", "y"), crs = "epsg:4326")
  tempdat   <- cbind(as.data.frame(dat, xy = TRUE),
  ...
  areas <- st_intersection(edc, marine_parks) %>%
    ...
    dplyr::mutate(depth_contour = case_when(
  ...
  # prevent many-to-many joins (same trick as benthos)
  ...
  if (any(tempdat$geoscience_depth < 0 & tempdat$geoscience_depth > -30, na.rm = TRUE)) {
    ...
    errors.shallow <- terra::extract(dat.shallow, marine_parks) %>%
      dplyr::group_by(ID) %>%
      dplyr::summarise(across(starts_with("p"), se), .groups = "drop") %>%
      dplyr::mutate(ID = as.character(ID), year = year) %>%
      dplyr::rename(cti_se = p_cti.fit, richness_se = p_richness.fit,
                    abundance_se = p_abundance.fit, b20_se = p_b20.fit) %>%
      dplyr::select(ID, year, cti_se, richness_se, abundance_se, b20_se)

    means.shallow <- terra::extract(dat.shallow, marine_parks) %>%
      dplyr::group_by(ID) %>%
      dplyr::summarise(across(starts_with("p"), ~mean(.x, na.rm = TRUE)), .groups = "drop") %>%
      dplyr::mutate(ID = as.character(ID), year = year) %>%
      dplyr::rename(cti = p_cti.fit, richness = p_richness.fit,
                    abundance = p_abundance.fit, b20 = p_b20.fit) %>%
      dplyr::select(ID, year, cti, richness, abundance, b20)

    out$shallow <- as.data.frame(marine_parks) %>%
      tibble::rownames_to_column() %>%
      dplyr::rename(ID = rowname) %>%
      left_join(errors.shallow, by = "ID") %>%
      left_join(means.shallow,  by = c("ID", "year")) %>%
      left_join(areas_shallow,  by = "zone") %>%
  ...
  if (any(tempdat$geoscience_depth < -30 & tempdat$geoscience_depth > -70, na.rm = TRUE)) {
    ...
    out$meso <- as.data.frame(marine_parks) %>%
      tibble::rownames_to_column() %>%
      dplyr::rename(ID = rowname) %>%
      left_join(errors.meso, by = "ID") %>%
      left_join(means.meso,  by = c("ID", "year")) %>%
      left_join(areas_meso,  by = "zone") %>%
      dplyr::filter(filter == "no") %>%
      dplyr::select(zone_new, status, year,
                    cti, cti_se, richness, richness_se,
                    abundance, abundance_se, b20, b20_se) %>%
  ...
  if (any(tempdat$geoscience_depth < -70 & tempdat$geoscience_depth > -200, na.rm = TRUE)) {
    ...
    out$rari <- as.data.frame(marine_parks) %>%
      tibble::rownames_to_column() %>%
      dplyr::rename(ID = rowname) %>%
      left_join(errors.rari, by = "ID") %>%
      left_join(means.rari,  by = c("ID", "year")) %>%
      left_join(areas_rari,  by = "zone") %>%
      dplyr::filter(filter == "no") %>%
      dplyr::select(zone_new, status, year,
                    cti, cti_se, richness, richness_se,
                    abundance, abundance_se, b20, b20_se) %>%
