...
![Temporal patterns in pH, Sea Level Anomaly, Sea Surface Temperature and Degree Heating Weeks](images/GeographeAMP_oceanography_time-series.png){#fig-temporalpressures}

![Sea level anomaly, as an index of upwelling, at the scale of the `{r} marine_park`. Heatmap values represent high (yellow) and low (blue) bimonthly sea level anomaly (SLA). Grey polygons represent State spatial management and Australian Marine Park boundaries. ](images/GeographeAMP_SLA.png){#fig-spatialsla}

![Spatial patterns in Sea Surface Temperature at the scale of the `{r} marine_park`.](images/GeographeAMP_SST.png){#fig-spatialsst}

![Spatial patterns in Degree Heating Weeks for 2 maximal timepoints](images/GeographeAMP_DHW.png){#fig-spatialdhw}
...
![Knowledge of the `{r} marine_park`](images/control-plots_knowledge-01.png){#fig-controlknowledge}

### Attitudes

![Attitudes towards the `{r} marine_park`](images/control-plots_attitudes-01.png){#fig-controlattitudes}
...
![Predicted dominant habitat for the `{r} marine_park`. State and Commonwealth marine park boundaries are shown.](images/GeographeAMP_predicted-dominant-habitat.png)

### Distribution of individual habitat classes

![Predictions for individual habitat classes for the `{r} marine_park`. State and Commonwealth marine park boundaries are shown.](images/GeographeAMP_predicted-individual-habitat.png)

![Temporal patterns in key benthic classes in the shallow ecosystem depth contour (0 - 30 m) `{r} marine_park`.](images/GeographeAMP_shallow-control-plots.png)

![Temporal patterns in key benthic classes in the mesophotic ecosystem depth contour (30 - 70 m) in the `{r} marine_park`.](images/GeographeAMP_mesophotic-control-plots.png)
...
![Temporal predictions for the shallow ecosystem depth contour (0 - 30 m) of key fish metrics for the `{r} marine_park`.](images/GeographeAMP_shallow_control-plots.png){#fig-fishtempshallow}

![Temporal predictions for the mesophotic ecosystem depth contour (30 - 70 m) of key fish metrics for the `{r} marine_park`.](images/GeographeAMP_mesophotic_control-plots.png){#fig-fishtempmeso}
