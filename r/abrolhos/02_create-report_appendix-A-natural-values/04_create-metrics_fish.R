...
# How many rows enter vs exit?
bind_rows(ta.sr, cti) %>% nrow()
nrow(tidy_maxn)

# # Which samples are dropped by the filter step?
# bind_rows(ta.sr, cti) %>%
#   dplyr::select(-c(log_count, w_sti)) %>%
#   dplyr::left_join(metadata) %>%
#   dplyr::left_join(benthos) %>%
#   dplyr::left_join(metadata_bathy_derivatives) %>%
#   filter(is.na(reef) | is.na(geoscience_aspect)) %>%
#   distinct(sample) %>%
#   print(n = Inf)
...
marine_parks_amp <- st_read("data/south-west network/spatial/shapefiles/western-australia_marine-parks-all.shp") %>%
  dplyr::filter(name %in% c("Abrolhos")) %>% #TODO AL added: change to relevant parks
