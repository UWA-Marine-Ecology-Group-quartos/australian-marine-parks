...
e <- ext(112, 115.4, -30, -27) # changed to be abrolhos, might need to double check
...
dir.create(paste0("data/", park, "/spatial/rasters/"), recursive = TRUE, showWarnings = FALSE) #AL added this in
...
metadata.bathy.derivatives   <- cbind(metadata,
  ...
  filter(if_all(c(geoscience_depth, geoscience_aspect, geoscience_roughness, geoscience_detrended),
                ~!is.na(.))) %>% # Checked, none are missing
  dplyr::select(-ID) %>%
  glimpse()

## AL maybe add checking code here hashed out?
# Check which samples are missing bathymetry derivatives
# missing_bathy <- cbind(metadata, terra::extract(preds, metadata_sf)) %>%
#   filter(if_any(c(geoscience_depth, geoscience_aspect, geoscience_roughness, geoscience_detrended),
#                 ~is.na(.)))
#
# nrow(missing_bathy)
# glimpse(missing_bathy)
