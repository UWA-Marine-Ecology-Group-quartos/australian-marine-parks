...
## detrended not supported for any habitat class. year removed - pooled benchmark.

# Sand - best model: aspect + depth + roughness
...
# Macroalgae - best model: aspect + depth + roughness
...
# Inverts - best model: aspect + depth + roughness
...
# Reef - best model: depth + roughness (simplest within delta AICc 2)
...
# reef included here so the fish script has a predicted reef layer to read
...
# reef excluded here - it's a predictor for fish, not a dominant seabed class
dom_rast <- benthos_dom_tag(
  terra::subset(preddf_m, c("p_sand.fit", "p_macro.fit", "p_inverts.fit"))
)
