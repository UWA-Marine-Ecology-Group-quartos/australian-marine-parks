...
# Spatial plots
## SST
sst <- rast(paste0("data/geographe/spatial/oceanography/", name, "_SST_raster.rds")) %>%
...
ggplot() +
  ...
  coord_sf(xlim = c(115.0526, 115.5551),
           ...
           crs = 4326)
ggsave(paste0("plots/geographe/spatial/", name, "_SST.png"),
       height = 4.5, width = 8, dpi = 600, bg = "white", units = "in")

## SLA
sla <- rast(paste0("data/geographe/spatial/oceanography/", name, "_SLA_raster.rds")) %>%
  subset(names(.) %in% c("Jan", "Mar", "May", "Jul", "Sep", "Nov"))
names(sla)
...
ggplot() +
  ...
  coord_sf(xlim = c(115.0526, 115.5551),
           ylim = c(-33.65736, -33.35236),
           crs = 4326)
ggsave(paste0("plots/geographe/spatial/", name, "_SLA.png"),
       height = 4.5, width = 8, dpi = 600, bg = "white", units = "in")

## DHW
dhw <- rast(paste0("data/geographe/spatial/oceanography/", name, "_DHW_raster.rds"))
names(dhw)
...
ggplot() +
  ...
  coord_sf(xlim = c(115.0526, 115.5551),
           ylim = c(-33.65736, -33.35236),
           crs = 4326)
ggsave(paste0("plots/geographe/spatial/", name, "_DHW.png"),
       height = 3.5, width = 8, dpi = 600, bg = "white", units = "in")

# Time series plots
## Acidification
acid_ts <- readRDS(paste0("data/geographe/spatial/oceanography/",
...
sla_ts <- readRDS(paste0("data/geographe/spatial/oceanography/",
...
sst_ts <- readRDS(paste0("data/geographe/spatial/oceanography/",
